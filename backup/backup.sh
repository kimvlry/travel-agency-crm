...
find_primary() {
    ...
    if [ -z "$primary" ]; then
        echo "REST API check failed, trying direct connections..." >&2
        for node in node1 node2; do
            echo "Testing direct connection to $node..." >&2
            if PGPASSWORD="$POSTGRES_PASSWORD" pg_isready -h "$node" -p 5432 -U "$POSTGRES_USER" >/dev/null 2>&1; then
                if PGPASSWORD="$POSTGRES_PASSWORD" psql -h "$node" -p 5432 -U "$POSTGRES_USER" -d postgres -t -c "SELECT NOT pg_is_in_recovery();" 2>/dev/null | grep -q 't'; then
                    primary=$node
                    echo "Found primary via direct connection: $primary" >&2
                    break
                fi
            fi
        done
    ...
    if [ -z "$primary" ]; then
        echo "Could not determine primary, using haproxy as fallback" >&2
        primary="haproxy"
    fi
...
echo "Using database host: $PRIMARY_HOST"

echo "Testing connection to $PRIMARY_HOST..."
if ! PGPASSWORD="$POSTGRES_PASSWORD" pg_isready -h "$PRIMARY_HOST" -p 5432 -U "$POSTGRES_USER"; then
    echo "ERROR: Cannot connect to database at $PRIMARY_HOST"
    exit 1
fi
