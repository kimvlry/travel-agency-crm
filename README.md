...
- `MIGRATION_VERSION` - версия, до которой выполняются миграции
- `APP_ENV` - режим запуска приложения (seeding выполняется в `dev`)
- `SEED_COUNT` - кол-во кортежей для нормировки сидирования 

- `ANALYST_NAMES` - список имен пользователей с ролью `analytic` (read-only)

- `BACKUP_RETENTION_COUNT` - кол-во единовременно хранимых бэкапов
- `BACKUP_INTERVAL_CRON` - частота выполнения бэкапа (cron, например: "*/1 * * * *")